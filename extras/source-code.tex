...
% lwarp sobreescreve o que foi definido em literate; aqui,
% modificamos para manter os valores anteriores e apenas
% acrescentar as novas substituições. Não dá para fazer
% isto em outro arquivo porque utilizamos o comando aqui mesmo.
\makeatletter
\warpHTMLonly{
\def\lstset@#1{\endgroup%
\ifx\@empty#1\@empty\else\setkeys{lst}{#1}\fi%
\providecommand{\lst@literate}{}%
        \edef\lst@literate{%
                \expandonce\lst@literate%
                {<}{\noexpand\HTMLentity{lt}}{4}%
                {>}{\noexpand\HTMLentity{gt}}{4}%
                {\&}{\noexpand\HTMLentity{amp}}{5}%
        }%
}
}
\makeatother
